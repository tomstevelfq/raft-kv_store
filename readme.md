...
## 问题3
节点失效时，如何操作以恢复数据，map节点失效如何做，reduce节点失效如何做，文件访问不上如何操作  rpc请求失败如何容错
文件进行一个简单的备份，文件访问不上先访问备份文件，备份文件访问不到就重新执行map任务

coordinator通过心跳检测判断节点超时，节点检测到超时无应答时，开始重新执行分配给它的map任务，
worker节点执行reduce时发现远程map输出中间键值对文件无法访问时，先访问它的备份文件，备份文件无法访问到时，该reduce任务终止执行，同时告知coordinator触发重新分配原节点的map任务

map完成任务时添加时间戳，区分是不是最新map result
